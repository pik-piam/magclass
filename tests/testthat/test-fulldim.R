...
test_that("fulldim works", {
  expect_warning(fd <- fulldim(a[1:2,1:2,]), "deprecated")
  ...
  expect_identical(suppressWarnings(fulldim(p0)), list(c(1L, 1L, 1L), list(i = "AFR", t = "y1995", d3 = NULL)))
  p2 <- p[1,1,]
  getItems(p2, dim = 3, raw = TRUE) <- c("A.B","C")
  expect_warning(fulldim(p2), "cannot be splitted")
  ...
  expect_identical(suppressWarnings(fulldim(a2[1:2,1,])), ref)
  ...
  expect_identical(suppressWarnings(fulldim(a3)), ref)
})
