...
test_that("getCoords can read and write coordinates", {
  ...
  expect_silent(getCoords(x) <- as.matrix(coords))
  expect_identical(getCoords(x), coords)
  expect_error(getCoords(x) <- 1:10, "unsupported format")
  expect_error(getCoords(x) <- head(coords), "Number of rows does not match")
