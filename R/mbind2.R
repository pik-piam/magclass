...
mbind2 <- function(...) {
  ...
  if (any(!ismagpie & !isnull)) stop("Input(s) ", paste(which(!ismagpie & !isnull), collapse = ", "),
    " is/are no MAgPIE objects(s)!")
  if (length(list(...)) == 1 & is.list(list(...)[[1]])) stop("Input is a list!") # stopped earlier (magpie-check)
  ...
  if (difftemp) {
    ...
    for (i in seq_along(list(...)[ismagpie])) output[, getYears(list(...)[ismagpie][[i]]), ] <- list(...)[ismagpie][[i]]
    return(output)
  } else {
    ...
    if (length(elems) != length(unique(elems))) {
      ...
      for (i in seq_along(lelems)) lelems[[i]] <- paste(lelems[[i]], i, sep = ".x")
      elems <- unlist(lelems)
      output <- new.magpie(getCells(list(...)[ismagpie][[1]]), getYears(list(...)[ismagpie][[1]]), elems)
      for (i in seq_along(list(...)[ismagpie])) {
      ...
      for (i in seq_along(list(...)[ismagpie])) {
        output[, , getNames(list(...)[ismagpie][[i]])] <- list(...)[ismagpie][[i]]
      }
    }
    return(output)
  }
