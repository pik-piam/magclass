...
setMethod("as.magpie",
  ...
  function(x, sep = ".", replacement = "_", filter = TRUE, ...) {
    isQuitte <- function(x, warn = FALSE) {
      ...
      if (!all(mandatoryColumns %in% names(x))) {
        if (warn) warning("Object formally defined as quitte object, but it does not contain all ",
          "required columns (missing: ", paste(mandatoryColumns[!(mandatoryColumns %in% names(x))],
            collapse = ", "), ")!")
        return(FALSE)
      }

      # are all columns factors which have to be factors?
      factorColumns <- sapply(x[c("model", "scenario", "region", "variable", "unit")], is.factor)
      if (!all(factorColumns)) {
        if (warn) warning("Object defined as quitte object, but there are columns not stored as factor which",
          " actually have to be stored that way (no factor: ",
          paste(names(factorColumns)[!factorColumns], collapse = ", "), ")!")
        return(FALSE)
      }
      ...
      if (!methods::is(x$period, "POSIXct") && !is.integer(x$period)) {
        if (warn) warning("Object defined as quitte object, but period column is neither integer nor of type POSIXct!")
        return(FALSE)
      }
