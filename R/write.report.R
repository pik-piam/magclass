...
#'
write.report <- function(x, file = NULL, model = "MAgPIE", scenario = "default", unit = NA, ndigit = 4,
                         append = FALSE, skipempty = TRUE) {
  if (is.list(x)) {
    if (is.list(x[[1]])) {
    ...
    for (mod in model) {
      for (scen in scenario) {
        ...
        if (length(unit) == 1) {
          nelemWithBrackets <- length(grep("\\(*\\)$", getNames(xtemp)))
          if (nelemWithBrackets == dim(xtemp)[3]) {
            tmp <- getNames(xtemp)
            dimnames(xtemp)[[3]] <- sub(" \\(([^\\(]*)\\)($|\\.)", "", tmp)
            unit <- sub("\\)$", "", sub(".* \\(", "", tmp))
          } else {
            if (nelemWithBrackets > 0) warning("Some but not all variable entries provide information in",
              " brackets which might be a unit information. To have it",
              " detected as unit all entries must provide this information!")
            unit <- rep(unit, ndata)
          }
        }
        ...
        for (i in 1:ndata) {
          if (ndim(x, dim = 3) == 3) {
            if (length(strsplit(getNames(xtemp)[i], split = "\\.")[[1]]) > 1                &
              ...
              output[(i - 1) * nregions + 1:nregions, "Variable"] <- strsplit(getNames(xtemp)[i],
                split = "\\.")[[1]][[3]]
            } else {
        ...
        if (is.null(file)) {
          ...
          if (append) {
            ...
            if (length(union) > length(years1)) {
              ...
              write.table(addycols(data, union), file, quote = FALSE, sep = ";", row.names = FALSE, col.names = TRUE,
                append = FALSE, eol = ";\n")
            }
          ...
          write.table(output, file, quote = FALSE, sep = ";", row.names = FALSE, col.names = !append,
            append = append, eol = ";\n")
          ii <- ii + 1
        }
        unit <- unitdef
