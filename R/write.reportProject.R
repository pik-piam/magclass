...
write.reportProject <- function(mif,mapping,file=NULL,max_file_size=NULL,format="default",...){
  ...
  if(!is.null(file)){
    ...
    if(format == "default") {
      ...
      b<-melt(a,id.vars = c("Model","Scenario","Region","Item","Variable","Unit"),variable.name = "Year")
      b$Year<-substr(b$Year,2,5)
      names(b)[which(names(b)=="value")]<-"Value"
      ...
      if(file_size > max_file_size) {
        ...
        for (f in 1:n_files) {
          ...
          if(format == "default") {
            ...
            b<-melt(a[[1]][[1]],id.vars = c("Model","Scenario","Region","Item","Variable","Unit"),variable.name = "Year")
            b$Year<-substr(b$Year,2,5)
            names(b)[which(names(b)=="value")]<-"Value"
            b<-b[c("Model","Scenario","Region","Item","Variable","Year","Unit","Value")]
