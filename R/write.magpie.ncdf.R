...
#'
#' @param x MAgPIE object. Has to be on half degree resolution. If x as comments in attr,
#' they are plotted as global attributes.
...
#' 3 format.
#' @param var_style change between variable naming in nc-file; "fullname" for ungrouped name,
#' "grouped" for variable names divided into sub-groups
#' @param comment Vector of comments (also used for setting the unit). Comments are
#' set as global attributes in the netcdf file. Format of comments: "indicator: comment"
#' (e.g. "unit: Share of land area per grid cell")
...
#' several files are written with the structure filename_year_datacolumn.asc. In the case
#' several data dimensions exist, they are saved as subcategories.
#'
...
#'
write.magpie.ncdf <- function(x, file, nc_compression = 9, var_style = "fullname", comment = NULL, verbose = TRUE) { # nolint
  ...
  if (!is.null(comment)) {
    ...
    beginWithSpace <- (regexpr(pattern = " ", text = indicator) == 1)
    if (any(beginWithSpace)) { # delete space at first place
      indicator[beginWithSpace] <- substring(indicator[beginWithSpace], first = 2)
    }
  }

  if (var_style == "grouped") getNames(x) <- gsub(pattern = "\\.", replacement = "/", getNames(x))
  ...
  if (length(getCells(mag)) == 59199) {
    ...
    if (!hasCoords(x)) {
      stop("Data has to be provided with 59199 cell or in half-degree data with",
        " cell names containing coordinate information.")
    }
  ...
  dimTime <- ncdf4::ncdim_def("time", "years", time, calendar = "standard")
