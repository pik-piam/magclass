...
magpie_expand <- function(x, ref) { # nolint
  ...
  if (!is.null(version) && version < 2) return(magpieExpand1(x, ref))
...
magpieExpand1 <- function(x, ref) {
  ...
  if (ncells(ref) > ncells(x)) {
    ...
    if (nregions(x) == ncells(x)) {
      if (nregions(x) == 1 & getRegions(x)[1] == "GLO") {
        ...
        # regional data
        if (nregions(x) != nregions(ref)) stop("Cannot expand MAgPIE object! Incompatible cells (different length).")
        if (any(sort(getRegions(x)) != sort(getRegions(ref)))) stop("Cannot expand MAgPIE object! Incompatible cells.")
      ...
      stop("Cannot expand MAgPIE object! Incompatible cells (different length).")
    }
    ...
    if (any(getCells(ref) != getCells(x))) {
      if (ncells(x) != 1 & any(sort(getCells(ref)) != sort(getCells(x)))) {
        ...
        if (getRegions(x) == "GLO") {
          getCells(x) <- getCells(ref)
        } else if (getRegions(ref) != "GLO" & getRegions(ref) != getRegions(x)) {
          stop("Region names do not agree! x:", getRegions(x), " ref:", getRegions(ref))
        }
      } else {
  ...
  if (nyears(ref) > nyears(x)) {
    if (nyears(x) == 1) {
      ...
      getSets(x, fulldim = FALSE)[2] <- ifelse(is.null(getSets(ref, fulldim = FALSE)[2]),
        "year", getSets(ref, fulldim = FALSE)[2])
    } else {
    ...
    if (is.null(getYears(x))) {
      ...
      if (any(sort(getYears(ref)) != sort(getYears(x)))) {
        if (nyears(x) == 1) {
          stop("Years do not agree! x:", paste(getYears(x), collapse = " "), " ref:",
            paste(getYears(ref), collapse = " "))
        } else {
          stop("Years do not agree! No mapping possible! x:", paste(getYears(x), collapse = " "),
            " ref:", paste(getYears(ref), collapse = " "))
        }
      } else {
  ...
  if (length(getNames(x)) == 0) {
    ...
    if (ndata(ref) > 1) stop("Inconsistent MAgPIE ref file: several elements in data dimension but no names given!")
    ...
    if (all(suppressWarnings(sort(getNames(x)) == sort(getNames(ref))))) {
      ...
      if (length(toadd) > 0) {
        ...
        getNames(x) <- newnames
        getSets(x, fulldim = FALSE)[3]  <- paste(getSets(x, fulldim = FALSE)[3],
          paste(names(rfdim)[toadd], collapse = "."), sep = ".")
        if (getOption("magclass.verbosity") > 1) {
          cat("NOTE (magpie_expand): data dimensionality of MAgPIE object expanded (added dimensions:",
            paste(rfdim[toadd]), ")\n")
        }
      }

      # is the order of the dimensions in the data dimension identical?
      # reorder if necessary (only performed if the number of dimensions is the same)
      xfdim <- .fulldatadim(x, TRUE)
      .tmp <- function(rfdim, xfdim) return(which(xfdim %in% list(rfdim)))
      order <- lapply(rfdim, .tmp, xfdim)
      lengthsOrder <- sapply(order, length)
      if (any(lengthsOrder == 0)) {
      ...
      if (any(lengthsOrder > 1)) {
        ...
        for (i in probs) {
          ...
          order[[i]] <- order[[i]][1]
          if (length(order[[i]]) == 0) {
            stop("Something went wrong in mapping correction in magpie_expand. Function seems to be bugged!")
          }
        }
      }
      order <- unlist(order)
      # add missing dimensions at the end
      order <- c(order, setdiff(seq_along(xfdim), order))

      if (any(order != seq_along(order))) {
        # different order
      ...
      if (ndata(x) == ndata(ref)) {
        if (length(xfdim) == length(rfdim)) {
          ...
          if (all(sort(getNames(x)) == sort(getNames(ref)))) {
            ...
            stop("Data names do not agree between ref and expanded x, magpie_expand seems to be bugged!")
          }
        } else {
          # more complicated case in which x has more dimensions than ref
          search <- paste0(paste0(rep("\\.[^\\.]*", length(xfdim) - length(rfdim)), collapse = ""), "$")
          reducedXnames <- sub(search, "", getNames(x))
          # in the case that now all data names of x and ref agree reorder the data best on order of ref
          if (all(sort(reducedXnames) == sort(getNames(ref)))) {
            x <- x[, , getNames(ref)]
          } else {
            stop("Data names do not agree between ref and expanded x, magpie_expand seems to be bugged!")
          }
